...
NTG Console locks USB capture at **0 dB**, runs those processors in software, and publishes an input named **`NTG_Console`**. The engine is a user service, so the virtual mic stays up after you close the window. It does not steal your default headset or speakers — pick `NTG_Console` in an app when you want it.
...
3. In the sound menu (or Zoom / Discord), pick input **`NTG_Console`**, not the raw “RØDE VideoMic NTG”.
...
Presets: **Calls** (75 Hz, RNNoise, gate, compressor), **Broadcast** (adds HF boost, Big Bottom, Exciter), **Raw** (straight through).

## Firmware

Firmware updates still need RØDE Central on Windows or Mac. This app does not flash the microphone.
